...
from uniforms.integrations.apps_script_api.script import AppsScript, Parameter
...
class AppsScriptService:
    ...
    @handle_http_errors
    def execute_script_by_name(self, script_name: str, args: List[Parameter]):
        if not self.scripts.get(script_name):
            raise NotRegisteredScript
        
        response = self.service.scripts().run(
            body=self.scripts.get(script_name).request(args),
            scriptId=self.scripts.get(script_name).id
        ).execute()
    ...
    def register(self, scripts: List[AppsScript]):
